"""
Persistence-based Watershed Algorithm on 2D and 3D images
Try flatten version
...
for xSub in range(len(ID)):
	for i in range(0,101):
		t0 = time.time()
		filename = "%s/%s/%s_%s_Weight_cropped_%s_r_%d.nii" % (ID[xSub], ROI_name[xROI], ROI_name[xROI], ID[xSub], Type[xT], i)		
		img = _load_3D_img(filename) # 3D nii
		img = img.get_fdata()
		# img = img[1:20,1:20,1:20]
		...
		# pdb.set_trace()
		filename2 = "%s/%s/cc/%s_%s_%s_cc_r_%d.mat" % (ID[xSub], ROI_name[xROI], ROI_name[xROI], ID[xSub], Type[xT], i)
		io.savemat(filename2, {"cc": img_cc})
		index = np.unique(img_cc)
		msg = "%s_%d: N cluster = %d, mean cluster size = %f, time = %f" % (ID[xSub], i, len(index) - 1, nVoxel/(len(index) - 1), time.time() - t0)
		print(msg)		
		#print(len(index))
		#print (time.time() - t0)